...
const codesPath = path.join(
  ...
  "../codes/baltimore-codes.json"
);

const BALTIMORE_CODES: Record<string, ViolationCode> = JSON.parse(
...
export const baltimoreAdapter: CityAdapter = {
  cityId: "baltimore",
  ...
  async lookupTickets(
    ...
    try {
      ...
      const plateInput = page.locator(
        "input[name*='plate' i], input[id*='plate' i], input[placeholder*='plate' i], input[placeholder*='license' i]"
      );
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        const description =
          cellTexts[3] ?? codeInfo.description ?? "Unknown violation";
        const amountStr = cellTexts[4] ?? "0";
        const amount =
          parseFloat(amountStr.replace(/[^0-9.]/g, "")) || codeInfo.fine;
  ...
  async getTicketDetails(violationNumber: string): Promise<TicketDetail> {
    ...
    try {
      ...
      const citationInput = page.locator(
        "input[name*='citation' i], input[id*='citation' i], input[placeholder*='citation' i], input[name*='issue' i], input[id*='issue' i], input[placeholder*='issue' i]"
      );
      ...
      for (const m of kvMatches) {
        const k = m[1].trim();
        if (k && !rawData[k]) rawData[k] = m[2].trim();
      }
  ...
  getDisputeFormStructure(): DisputeFormFields {
    return {
      ...
      notes:
        "Baltimore City parking ticket disputes are handled by phone or in person — " +
  ...
  async submitDispute(
    ...
    throw new Error(
      `Automated dispute submission is not supported for Baltimore City. ` +
