...
async function queryApi(params: string): Promise<SfCitation[]> {
  ...
  return resp.json() as Promise<SfCitation[]>;
}
...
function citationToTicket(c: SfCitation): Ticket {
  return {
...
async function scrapeEtims(
  ...
  try {
    ...
    const submitBtn = page.locator(
      "input[type='submit'], button[type='submit'], input[value='Search' i], button:has-text('Search')"
    );
...
export const sfAdapter: CityAdapter = {
  ...
  async lookupTickets(
    ...
    const where = encodeURIComponent(
      `vehicle_plate='${plate.toUpperCase()}' AND vehicle_plate_state='${state.toUpperCase()}'`
    );
    const apiResults = await queryApi(
      `$where=${where}&$order=citation_issued_datetime DESC&$limit=50`
    );
  ...
  async getTicketDetails(violationNumber: string): Promise<TicketDetail> {
    const results = await queryApi(
      `citation_number=${encodeURIComponent(violationNumber)}`
    );
  ...
  getDisputeFormStructure(): DisputeFormFields {
    return {
      ...
      notes:
        "SF parking citation contests are filed through SFMTA. You have 21 " +
  ...
  async submitDispute(
    ...
    throw new Error(
      "Automated dispute submission is not supported for San Francisco. " +
