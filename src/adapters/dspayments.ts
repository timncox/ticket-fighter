...
export function createDsPaymentsAdapter(config: {
  ...
  displayName: string;
  citySlug: string;
  portalDomain?: string; // "dspayments.com" (default) or "dsparkingportal.com"
  appealSlug?: string; // for duncan.imageenforcement.com/adminreviewsites/{slug}
}): CityAdapter {
    ...
    async lookupTickets(plate: string, state: string, _type: string): Promise<Ticket[]> {
      ...
      try {
        ...
        const plateInput = page
          .locator(
            'input[name*="plate" i], input[id*="plate" i], input[placeholder*="plate" i], input[name*="license" i], input[id*="license" i]'
          )
          .first();
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const amount =
            parseFloat((cellTexts[5]?.trim() ?? "0").replace(/[^0-9.]/g, "")) || 0;
    ...
    async getTicketDetails(violationNumber: string): Promise<TicketDetail> {
      ...
      try {
        ...
        const ticketInput = page
          .locator(
            'input[name*="ticket" i], input[id*="ticket" i], input[name*="citation" i], input[id*="citation" i], input[name*="violation" i], input[id*="violation" i], input[type="text"]'
          )
          .first();
        ...
        const dateIssued =
          cellData[1] ?? rawData["Date"] ?? rawData["Issue Date"] ?? rawData["Date Issued"] ?? "";
        const violationCode =
          cellData[2] ?? rawData["Code"] ?? rawData["Violation Code"] ?? "";
        const description =
          cellData[3] ?? rawData["Description"] ?? rawData["Violation"] ?? violationCode;
        const status = cellData[4] ?? rawData["Status"] ?? "unknown";
        const amount =
          parseFloat(
            (cellData[5] ?? rawData["Amount"] ?? rawData["Fine"] ?? rawData["Balance Due"] ?? "0").replace(
              /[^0-9.]/g,
              ""
            )
          ) || 0;
        const location =
          cellData[6] ?? rawData["Location"] ?? rawData["Address"] ?? rawData["Street"] ?? "";
        const plate =
          cellData[7] ??
          rawData["Plate"] ??
