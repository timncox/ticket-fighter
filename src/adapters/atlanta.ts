...
const DUNCAN_APPEAL_URL =
  "https://duncan.imageenforcement.com/adminreviewsites/atlantareview";
...
const codesPath = path.join(
  ...
  "../codes/atlanta-codes.json"
);

const ATLANTA_CODES: Record<string, ViolationCodeEntry> = JSON.parse(
  fs.readFileSync(codesPath, "utf-8")
);

function getCodeInfo(code: string): ViolationCodeEntry {
  return (
    ATLANTA_CODES[code] ?? { description: "Unknown violation", fine: 0, defenses: [] }
  );
}

export const atlantaAdapter: CityAdapter = {
  cityId: "atlanta",
  displayName: "Atlanta",
  ...
  async lookupTickets(plate: string, state: string, _type: string): Promise<Ticket[]> {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const amount =
          parseFloat((cellTexts[4]?.trim() ?? "0").replace(/[^0-9.]/g, "")) || 0;
  ...
  async getTicketDetails(violationNumber: string): Promise<TicketDetail> {
    ...
    try {
      ...
      const ticketInput = page.locator(
        'input[name*="ticket" i], input[id*="ticket" i], input[name*="citation" i], input[id*="citation" i], input[placeholder*="ticket" i], input[placeholder*="citation" i]'
      ).first();
  ...
  async submitDispute(
    ...
    try {
      ...
      const citationInput = page.locator(
        'input[name*="citation" i], input[id*="citation" i], input[name*="ticket" i], input[id*="ticket" i]'
      ).first();
