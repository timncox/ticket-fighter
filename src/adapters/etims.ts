...
async function promptAndWaitForResults(
  page: import("playwright").Page,
  displayName: string,
  context: string
): Promise<void> {
  console.error(
    `[ticket-fighter] ${displayName} eTIMS CAPTCHA: A custom image CAPTCHA is displayed in the browser window.\n` +
...
async function parseTicketTable(
  page: import("playwright").Page,
  plate: string,
  cityId: CityId
): Promise<Ticket[]> {
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    if (!violationNumber || !/\d/.test(violationNumber)) continue;
...
export function createEtimsAdapter(config: {
  cityId: string;
  displayName: string;
  cityPath: string; // e.g. "sanfrancisco", "detroit", "cleveland"
  subdomain?: string; // "prodpci" (default), "wmq", or "pci"
  disputeUrl?: string; // city-specific dispute URL if known
}): CityAdapter {
  const cityId = config.cityId as CityId;
  ...
  return {
    ...
    async lookupTickets(
      ...
      try {
        ...
        const plateTab = page.locator(
          "a:has-text('License Plate'), button:has-text('License Plate'), " +
        ...
        await promptAndWaitForResults(
          page,
          config.displayName,
          `plate lookup: ${plate} (${state.toUpperCase()})`
        );
    ...
    async getTicketDetails(violationNumber: string): Promise<TicketDetail> {
      ...
      try {
        ...
        const citationTab = page.locator(
          "a:has-text('Citation'), button:has-text('Citation'), " +
        ...
        const citationInput = page.locator(
          "input[name*='citation' i], input[id*='citation' i], input[name*='ticket' i], " +
        ...
        await promptAndWaitForResults(
          page,
          config.displayName,
          `citation lookup: ${violationNumber}`
        );
    ...
    getDisputeFormStructure(): DisputeFormFields {
      ...
      return {
        ...
        notes:
          `${config.displayName} parking citations are processed through the eTIMS portal ` +
    ...
    async submitDispute(
      ...
      throw new Error(
        `Automated dispute submission is not supported for ${config.displayName} because the eTIMS portal ` +
    ...
    async checkDisposition(violationNumber: string): Promise<DisputeStatus> {
      ...
      try {
        ...
        await promptAndWaitForResults(
          page,
          config.displayName,
          `disposition check: ${violationNumber}`
        );
        ...
        const dateMatch = pageText.match(
          /(?:decision|decided|hearing|adjudication)\s*(?:date)?[:\s]+(\d{1,2}\/\d{1,2}\/\d{2,4})/i
        );
        if (dateMatch) {
