...
const DC_CODES: Record<string, ViolationCode> = JSON.parse(
  fs.readFileSync(codesPath, "utf-8")
);

function getCodeInfo(code: string): ViolationCode {
...
async function promptAndWaitForResults(
  ...
  console.error(
    `[ticket-fighter] DC eTIMS CAPTCHA: A custom image CAPTCHA is displayed in the browser window.\n` +
...
export const dcAdapter: CityAdapter = {
  ...
  getDisputeFormStructure(): DisputeFormFields {
    return {
      ...
      notes:
        "Washington DC parking disputes are handled by the DC DMV Adjudication Services. " +
  ...
  async submitDispute(
    ...
    throw new Error(
      `Automated dispute submission is not supported for Washington DC because the eTIMS portal ` +
