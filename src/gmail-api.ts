...
async function gmailGet(endpoint: string): Promise<unknown> {
 ...
 */
export async function searchGmailApi(query: string): Promise<GmailSearchResult> {
  ...
  const emails: GmailSearchResult["emails"] = [];
  ...
  for (const msgRef of list.messages.slice(0, 5)) {
    const msg = (await gmailGet(`messages/${msgRef.id}?format=full`)) as GmailMessage;
    ...
    for (const pdf of pdfParts) {
      try {
        const attachment = (await gmailGet(
          `messages/${msgRef.id}/attachments/${pdf.attachmentId}`,
        )) as { data: string };
